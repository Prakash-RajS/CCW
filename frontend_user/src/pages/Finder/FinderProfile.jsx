...
//     return collaborator?.id || collaborator?.user_id || collaborator?.userId;
//   };
...
//           const invitedMap = {};
//           sentByMe.forEach(inv => { 
//             if (inv.job_id) invitedMap[inv.job_id] = true; 
//           });
...
//       const activeJobs = jobs.filter(job => job.status === "posted" || job.status === "active");
//       const searchLower = searchTerm.toLowerCase();
//       setFilteredJobs(activeJobs.filter(job =>
...
//     if (!collaboratorId) {
//       console.error('No collaborator ID found:', collaborator);
//       toast.error("Unable to identify collaborator. Please try again.");
//       return;
//     }
...
//       toast.error(`Cannot send invitation: "${selectedJob.title}" already has a contract.`); 
//       return; 
//     }
//     if (invitedJobs[selectedJobId]) { 
//       toast.error(`Cannot send invitation: ${collaborator.name?.split(' ')[0] || 'User'} has already been invited to "${selectedJob.title}".`); 
//       return; 
//     }
...
//           jobTitle: selectedJob.title || 'Project'
//         });
//         setShowSuccessPopup(true);

//         // Close the invite popup after a delay
//         setTimeout(() => { 
//           resetForm(); 
//           onClose(); 
//         }, 500);
...
//     }
//     if (isInvited) { 
//       toast.info(`${collaborator.name?.split(' ')[0] || 'User'} already invited to "${job?.title}".`); 
//       return; 
//     }
...
//         collaboratorName={successData.collaboratorName}
...
//                   <img
//                     src={getPopupProfileImage()}
//                     alt={collaborator.name}
//                     className="w-full h-full object-cover object-top"
//                     onError={(e) => { e.target.src = ManImg; }}
//                   />
//                 </div>
...
//                     <span className="text-white/90 text-xs font-medium">${collaborator.pricing_amount || 0}/hr</span>
...
//             {/* Body */}
//             <div className="p-4">
//               <h4 className="text-base font-semibold text-[#2A1E17] mb-1">Select a job to invite</h4>
//               <p className="text-xs text-gray-600 mb-3">Search for a job to invite {collaborator.name?.split(' ')[0] || 'User'} to collaborate on.</p>
...
//                   className="w-full px-3 py-2 pl-8 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#51218F] focus:border-[#51218F] outline-none transition"
//                   autoFocus 
//                 />
...
//                       <p className="text-gray-500 text-sm">No jobs match "<span className="font-medium">{searchTerm}</span>"</p>
//                     </div>
//                   ) : (
...
//                           const isSelectable = !hasContract && !isInvited;
//                           return (
//                             <div 
//                               key={job.id}
//                               className={`p-3 transition ${!isSelectable ? 'bg-gray-50 cursor-not-allowed opacity-70' : isSelected ? 'bg-purple-50 cursor-pointer' : 'hover:bg-gray-50 cursor-pointer'}`}
//                               onClick={() => handleJobSelect(job.id, hasContract, isInvited)}
//                             >
//                               <div className="flex items-start gap-2">
//                                 <div className="flex-1 min-w-0">
//                                   <h5 className="font-medium text-sm text-[#2A1E17] truncate">{job.title}</h5>
//                                   <p className="text-xs text-gray-500 mt-0.5" style={{ display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
//                                     {job.description?.substring(0, 60) || "No description available"}{job.description?.length > 60 ? "..." : ""}
//                                   </p>
//                                   <div className="flex items-center gap-2 mt-1">
//                                     <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded-full">{job.budget_type === "fixed" ? "Fixed" : "Hourly"}</span>
//                                     {job.budget_from && job.budget_to && <span className="text-[10px] text-gray-600">${job.budget_from} - ${job.budget_to}</span>}
//                                   </div>
//                                 </div>
//                                 <div className="flex gap-1 flex-shrink-0">
//                                   {hasContract && <span className="px-1.5 py-0.5 bg-red-100 text-red-800 text-[9px] rounded-full whitespace-nowrap">Contract</span>}
//                                   {isInvited && !hasContract && <span className="px-1.5 py-0.5 bg-yellow-100 text-yellow-800 text-[9px] rounded-full whitespace-nowrap">Invited</span>}
//                                   {isSelected && isSelectable && <span className="px-1.5 py-0.5 bg-purple-100 text-purple-800 text-[9px] rounded-full whitespace-nowrap">Selected</span>}
//                                 </div>
//                               </div>
//                             </div>
//                           );
...
//               {!hasStartedTyping && (
//                 <div className="text-center py-6 text-gray-400 border border-dashed border-gray-300 rounded-lg mb-3">
//                   <svg className="w-8 h-8 mx-auto mb-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
//                   </svg>
//                   <p className="text-xs">Type to search your jobs</p>
//                 </div>
//               )}
...
//                   className="flex-1 px-3 py-2 rounded-lg border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//                 >
//                   Cancel
//                 </button>
//                 <button 
...
//   // Fetch user's jobs for invite popup
//   useEffect(() => {
...
//             id: id,
//           };
//           console.log('Collaborator data with ID:', dataWithId);
//           setCollaboratorData(dataWithId);
...
//                 skills: response.data.skills || response.data.skill_set || [],
...
//     if (item.is_fallback) return;
//     if (item.file_url) {
//       const fullUrl = item.file_url.startsWith('http') ? item.file_url : `${API_BASE_URL}${item.file_url}`;
...
//     console.log('handleInvite called with:', { collaboratorId, jobId });
...
//       if (!collaboratorId) {
//         console.error('No collaborator ID');
//         toast.error("No collaborator selected");
//         return false;
//       }
...
//       // Success toast will be shown, but we'll also show the success popup
...
//     console.log('openInvitePopup - collaboratorData:', collaboratorData);
...
//     const collaboratorForPopup = {
//       ...collaboratorData,
//       id: collaboratorData?.id || id,
//     };

//     console.log('Opening popup with collaborator:', collaboratorForPopup);
//     setInvitePopup({ isOpen: true, collaborator: collaboratorForPopup });
//   };
...
//       openInvitePopup();
//     } else {
//       alert("Please log in to send a collaborate request");
//     }
//   };
...
//     e.target.src = [Ui1, Ui2, Ui3][index % 3];
//   };
...
//         <button
//           onClick={() => navigate(-1)}
//           className="flex items-center gap-2 px-4 py-2 text-[#51218F] hover:text-[#3d1768] transition-colors group"
//         >
...
//                 Hello!
//               </p>
//               <svg className="absolute -top-10 -right-10 w-[55px] h-[55px] z-0" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
...
//               </h1>
//               <img src={BigScribble} alt="ui-scribble" className="absolute -left-[80px] top-[100px] w-[65px]" />
//             </div>
...
//               <div className="absolute inset-0 rounded-[50px] p-[5px] bg-[#FFFFFF1A] !border !border-white border-[2px] [border-style:solid] [border-image-slice:1] [border-image-width:2] [border-image-repeat:stretch] [border-image-source:radial-gradient(150.36%_227.1%_at_8.75%_100%,#FFFFFF_0%,rgba(169,163,163,0)_100%)] backdrop-blur-[1px]"></div>
...
//               <button
//                 className="relative z-10 w-full h-full rounded-[50px] flex items-center justify-center gap-4 text-white text-[26px] font-semibold -mt-2 hover:opacity-90 transition-opacity"
...
//                 <>{collaboratorData?.name || "Sivaselvam"}'s exceptional {collaboratorData?.skill_category?.toLowerCase() || "product design"}<br />ensures our website's success.<br />Highly Recommended</>
//               ) : collaboratorData?.skills_rating >= 3 ? (
//                 <>{collaboratorData?.name || "Sivaselvam"}'s professional {collaboratorData?.skill_category?.toLowerCase() || "design"}<br />delivered quality work on time.<br />Would recommend</>
//               ) : (
//                 <>{collaboratorData?.name || "Sivaselvam"} completed the {collaboratorData?.skill_category?.toLowerCase() || "project"}<br />as per the requirements.<br />Satisfactory work</>
//               )}
//             </p>
//           </div>

//           {/* RIGHT EXPERIENCE */}
//           <div className="absolute right-[100px] top-[560px] w-[169px] flex flex-col items-center gap-[21px]">
//             <div className="flex gap-1">
//               {[...Array(5)].map((_, i) => (
//                 <span key={i} className={`text-[32px] ${i < Math.floor(starRating) ? "text-orange-500" : i === Math.floor(starRating) && starRating % 1 > 0.5 ? "text-orange-500" : "text-gray-300"}`}>★</span>
//               ))}
//             </div>
//             <div className="text-center">
...
//         {/* PORTFOLIO SECTION */}
//         <section className="relative w-full h-[900px] bg-cover bg-center rounded-[45px] -mt-4" style={{ backgroundImage: `url(${ServicesBG})` }}>
...
//             <h2 className="text-[48px] mt-44 ml-8 font-semibold text-white">My <span className="ml-1 text-[#51218F]">Portfolio</span></h2>
//             <p className="text-white text-[16px] w-[480px] mt-40 leading-[26px] relative right-[60px]">
//               {collaboratorData?.about?.substring(0, 150) || "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis lacus nunc, posuere in justo vulputate, bibendum sodales."}
//             </p>
//           </div>
//           <div className="relative w-[1099px] h-[462px] mx-auto mt-[190px]">
//             {showCarouselControls && (
//               <>
//                 <button onClick={prevSlide} className="absolute left-[-60px] top-1/2 -translate-y-1/2 z-30 w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white/30 transition-all">
//                   <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
//                 </button>
//                 <button onClick={nextSlide} className="absolute right-[-60px] top-1/2 -translate-y-1/2 z-30 w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white/30 transition-all">
...
//                 <div key={item.id} className="transition-all duration-300 w-[413px] opacity-100 transform hover:scale-[1.02] cursor-pointer" onClick={() => openFileInNewTab(item)}>
...
//                     <div className="relative z-10 w-full h-full rounded-[55px] bg-white/5 backdrop-blur-sm p-7 shadow-[0_0_40px_rgba(162,95,255,0.25)] hover:shadow-[0_0_60px_rgba(162,95,255,0.4)] transition-shadow">
...
//                         {item.file_type !== "image" && item.file_type !== "unknown" && !item.is_fallback && (
//                           <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-xl z-10">
//                             <svg className="w-16 h-16 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
//                           </div>
//                         )}
//                         <img src={item.is_fallback ? item.file_url : (item.file_type === "image" ? item.file_url : Ui1)} alt={item.title} className="relative rounded-xl shadow-xl w-full h-full object-cover" onError={(e) => handleImageError(e, index)} />
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
//           )}
//         </section>
//         {/* After the portfolio section, add skills showcase */}
...
//     <span className="text-[#334155]">Skills & </span>
//     <span className="text-[#6D28D9]">Expertise</span>
//   </h2>
...
//         <div
//           key={index}
//           className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-full px-6 py-3 shadow-md hover:shadow-lg transition-all hover:scale-105 cursor-pointer"
//         >
...
//                         <div className="absolute inset-[22px] rounded-full" style={{ background: color }} />
//                       </div>
...
//               <p className="text-[14px] bg-white px-5 py-[6px] rounded-full font-medium border-[2.5px] border-black" style={{ boxShadow: "0 0 0 2px black" }}>Hello!</p>
...
//               <h2 className="text-[32px] font-bold leading-[38px]">{collaboratorData?.skill_category || "Software"}</h2>
...
//                   {collaboratorData?.skills_rating >= 4
//                     ? `${collaboratorData?.name || "Sivaselvam"}'s exceptional ${collaboratorData?.skill_category?.toLowerCase() || "design"} ensures success.`
//                     : collaboratorData?.skills_rating >= 3
//                     ? `${collaboratorData?.name || "Sivaselvam"}'s professional work delivered quality.`
//                     : `${collaboratorData?.name || "Sivaselvam"} completed the work satisfactorily.`}
//                 </p>
//               </div>

//               <div className="relative w-full h-[360px] flex justify-center">
//                 <img src={HalfCircle} alt="half-circle" className="absolute bottom-[140px]" style={{ width: "355px", height: "296px" }} />
//                 <img src={ManImg} alt="man" className="absolute" style={{ width: "188px", bottom: "170px" }} />
//               </div>

//               <div className="absolute right-[10px] top-[8px] w-[115px] text-center">
//                 <div className="flex justify-center gap-[2px] mb-[2px]">
//                   {[...Array(5)].map((_, i) => (
//                     <span key={i} className={`text-[14px] ${i < Math.floor(starRating) ? "text-orange-500" : "text-gray-300"}`}>★</span>
//                   ))}
//                 </div>
//                 <h3 className="text-[18px] font-bold leading-none">{experienceDisplay}</h3>
//               </div>
//             </div>
...
//                 <div className="absolute inset-[7px] rounded-full bg-[radial-gradient(circle_at_30%_0%,#9D5CFF,#1A082F)] shadow-[0_0_35px_rgba(157,92,255,0.9)]" />
//                 <button
...
//         {/* MOBILE PORTFOLIO */}
//         <section className="relative w-full mt-[-81px] px-4 py-14 rounded-[28px] overflow-hidden" style={{ backgroundImage: `url(${ServicesBG})`, backgroundSize: "cover" }}>
...
//             <h2 className="text-[24px] font-semibold leading-tight">My <span className="text-[#7C3AED]">Portfolio</span></h2>
...
//             <div className="flex overflow-x-auto pb-8 scrollbar-hide gap-4">
//               {portfolioItems.map((item, index) => (
//                 <div key={item.id} className="min-w-[85%] rounded-[24px] p-4 bg-white/10 backdrop-blur-sm cursor-pointer" onClick={() => openFileInNewTab(item)}>
//                   <div className="rounded-[18px] p-2">
//                     <h3 className="text-white text-[16px] font-medium mb-3 text-center">{item.title || "Portfolio Item"}</h3>
//                     {item.file_type !== "image" && item.file_type !== "unknown" && !item.is_fallback && (
//                       <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-xl z-10">
//                         <svg className="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
//                       </div>
//                     )}
//                     <img src={item.is_fallback ? item.file_url : (item.file_type === "image" ? item.file_url : Ui1)} alt={item.title} className="w-full h-[200px] rounded-[14px] object-cover" onError={(e) => handleImageError(e, index)} />
//                   </div>
//                 </div>
//               ))}
//             </div>
...
//           <h2 className="text-center text-[28px] font-semibold mb-16">My <span className="text-[#6D28D9]">Work Experience</span></h2>
//           {workExperiences.length > 0 ? (
//             <div className="relative">
...
//                 const color = colors[idx % colors.length];
//                 return (
//                   <div key={exp.id || idx} className="grid grid-cols-[1fr_60px_1fr] items-start mb-20">
//                     <div className="text-right pr-3">
//                       <h3 className="text-[16px] font-semibold text-[#334155]">{exp.company_name}</h3>
//                       {exp.location && <p className="text-[12px] mt-1 text-[#9CA3AF]">{exp.location}</p>}
//                       <p className="text-[12px] text-[#9CA3AF] mt-1">{exp.start_year} – {exp.end_year || (exp.is_current ? "Present" : "")}</p>
//                     </div>
//                     <div className="flex justify-center"><TimelineNode color={color} /></div>
//                     <div className="text-left pl-3">
//                       <h4 className="text-[15px] font-semibold text-[#334155]">{exp.role}</h4>
//                       {exp.description && <p className="text-[12px] text-[#9CA3AF] mt-1 leading-[18px]">{exp.description.substring(0, 100)}{exp.description.length > 100 ? "..." : ""}</p>}
//                     </div>
//                   </div>
//                 );
//               })}
//             </div>
//           ) : (
//             <div className="text-center text-[24px] font-medium text-[#9CA3AF] py-10">No work experience yet</div>
//           )}
//         </section>

//         {/* MOBILE DISCUSS */}
//         <div className="mt-[-55px] px-6 text-center">
//           <h2 className="text-[21px] font-bold">Have an Awesome Project Idea?<span className="text-purple-600 block">Let's Discuss</span></h2>
...
//               <div className="absolute inset-[8px] rounded-full bg-[radial-gradient(circle_at_top,#8A38F5,#2A0A4F)] shadow-[0_0_30px_#8A38F5]"></div>
//               <button className="relative z-10 w-full h-full flex items-center justify-center text-white text-[16px] font-semibold hover:opacity-90" onClick={handleMessageClick}>Message me</button>
//             </div>
//           </div>
...
//                 <path d="M24 17L26.5 22H32L27.8 25.8L29.2 31.2L24 28L18.8 31.2L20.2 25.8L16 22H21.5L24 17Z" fill="#2A3442"/>
//               </svg>
//               <p className="text-[#2A3442] text-[12px] font-medium">{starRating.toFixed(1)}/5 Ratings</p>
//             </div>
...
//                 <circle cx="12" cy="12" r="3" fill="#2A3442"/>
//               </svg>
//               <p className="text-[#2A3442] text-[12px] font-medium">{collaboratorData?.review_count || 0}+ Reviews</p>
//             </div>
...
//                 <path d="M9.5 12.2L12 14.2L15.5 10" stroke="#2A3442" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
//               </svg>
//               <p className="text-[#2A3442] text-[12px] font-medium">Certified</p>
//             </div>
//           </div>
//         </div>
//       </div>
      ...
      <div className="hidden lg:block">
        ...
        <section className="relative w-full h-[900px] bg-cover bg-center rounded-[45px] -mt-4" style={{ backgroundImage: `url(${ServicesBG})` }}>
          ...
          <div className="relative w-[1099px] h-[462px] mx-auto mt-[190px]">
            ...
            <div className="flex justify-between gap-[29px]">
              {currentItems.map((item, index) => (
                <div key={item.id} className="transition-all duration-300 w-[413px] opacity-100 transform hover:scale-[1.02]">
                  <div className="relative w-full h-full rounded-[55px]">
                    <div className="absolute inset-0 rounded-[55px] border border-white/40 pointer-events-none z-20" />
                    <div className="relative z-10 w-full h-full rounded-[55px] bg-white/5 backdrop-blur-sm p-7 shadow-[0_0_40px_rgba(162,95,255,0.25)] hover:shadow-[0_0_60px_rgba(162,95,255,0.4)] transition-shadow">
                      <h3 className="text-white text-[24px] font-medium mb-5">{item.title || "Portfolio Item"}</h3>
      ...
      <div className="block lg:hidden">
        ...
        <section className="w-full relative bg-white overflow-hidden pb-8">
          ...
          <div className="relative mt-8 flex justify-center px-4">
            <div className="absolute left-2 top-2 w-[110px] text-left z-10">
              <div className="text-[32px] font-bold leading-none text-[#2A3442]">''</div>
              <p className="text-[11px] leading-[14px] font-bold text-[#2A3442] mt-1">
                {collaboratorData?.skills_rating >= 4
            ...
            <div className="relative w-full h-[320px] flex justify-center">
              <img src={HalfCircle} alt="half-circle" className="absolute bottom-[120px] w-[320px] h-[260px]" />
              <img src={ManImg} alt="man" className="absolute w-[170px] bottom-[140px] z-20" />
            </div>

            {/* ===== MOBILE RIGHT: STARS + EXPERIENCE (SINGLE BLOCK) ===== */}
            <div className="absolute right-0 top-10 w-[110px] text-center z-10">
              {starRating > 0 && (
                <div className="flex justify-center gap-[2px] mb-2">
                  {[...Array(5)].map((_, i) => (
                    <span key={i} className={`text-[14px] ${i < Math.floor(starRating) ? "text-orange-500"
                      : i === Math.floor(starRating) && starRating % 1 >= 0.5 ? "text-orange-500"
                        : "text-gray-300"
                      }`}>★</span>
                  ))}
              ...
              <h3 className="text-[16px] font-bold leading-tight">{experienceDisplay}</h3>
              {!isFresher && <p className="text-[11px] text-gray-500">Experience</p>}
            </div>
          </div>
        ...
        <section className="relative w-full mt-2 px-4 py-8 rounded-[32px] overflow-hidden" style={{ backgroundImage: `url(${ServicesBG})`, backgroundSize: "cover" }}>
          ...
          <div className="relative z-10 flex items-start justify-between text-white mb-6">
            <h2 className="text-[26px] font-semibold">My <span className="text-[#7C3AED]">Portfolio</span></h2>
          </div>
