...
export default function CompletedContracts() {
  ...
  const [downloadingMilestone, setDownloadingMilestone] = useState(null); // Track which milestone is downloading
  ...
  const downloadMilestoneWork = async (contract, milestoneIndex) => {
    ...
    // Set downloading state for this specific milestone
    setDownloadingMilestone(`${contract.id}-${milestoneIndex}`);
  ...
  // Check if contract has single file attachment
  const hasSingleFile = (contract) => {
    return contract.has_attachment || contract.external_file_link;
  };

  // Render milestone summary for completed contracts
  // Update the renderMilestoneSummary function with better spacing
const renderMilestoneSummary = (contract) => {
  ...
  return (
    <div className="mt-3 pt-3 border-t-2 border-purple-200 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-3">
      <div 
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-semibold text-purple-700">Total: ₹{totalAmount}</span>
          <svg 
      ...
      {isExpanded && (
        <div className="mt-3 space-y-3">
          {milestones.map((milestone, idx) => {
            const isDownloading = downloadingMilestone === `${contract.id}-${idx}`;
            
            return (
              <div key={idx} className="flex flex-col p-4 bg-white rounded-lg border border-purple-100 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 flex-1">
                    <span className={`w-3 h-3 rounded-full ${milestone.status === 'paid' ? 'bg-green-500' : 'bg-gray-400'}`} />
                    <span className="text-gray-800 font-semibold">Milestone {idx + 1}: {milestone.description}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-purple-700 font-bold">₹{milestone.amount}</span>
                ...
                {milestone.submission && (
                  <div className="mt-3 pl-6 space-y-2 bg-gray-50 p-3 rounded-lg">
                    {/* Description */}
                    {milestone.submission.description && (
                      <p className="text-sm text-gray-700 font-medium">📝 {milestone.submission.description}</p>
                    )}
                    ...
                    {milestone.submission.external_link && (
                      <div className="flex items-start gap-2">
                        ...
                        <a
                          ...
                          className="text-purple-600 text-sm underline break-all hover:text-purple-800"
                        >
                    ...
                    {milestone.submission.attachment && (
                      <div className="flex items-center gap-2 pt-1">
                        <button
                          ...
                          className={`bg-blue-500 text-white px-4 py-1.5 rounded-lg text-sm hover:bg-blue-600 transition-colors inline-flex items-center gap-2 ${
  ...
  // Render single file download buttons
  const renderSingleFileButtons = (contract) => {
    if (hasMilestones(contract)) return null; // Don't show for milestone projects
    
    const isDownloading = downloading === contract.id;
    
    return (
      <div className="absolute top-2 right-2 md:top-4 md:right-4 flex gap-1.5 md:gap-3">
        {contract.has_attachment && (
          <div
            onClick={() => !isDownloading && downloadFile(contract.id, contract.has_attachment)}
            className={`w-7 h-7 md:w-[52px] md:h-[52px] rounded-full bg-gradient-to-br from-[#7C3AED] to-[#2B0F4C] flex items-center justify-center shadow-[0_6px_20px_rgba(124,58,237,0.35)] cursor-pointer hover:opacity-90 transition ${
  ...
  const downloadFile = async (contractId, hasAttachment) => {
    ...
    setDownloading(contractId); // Set downloading state
        ...
        {loading ? (
          ...
          <>
            {currentContracts.map((contract) => {
              ...
              const isMilestoneProject = hasMilestones(contract);

              return (
                <div
                  key={contract.id}
                  className={`relative border-2 rounded-xl md:rounded-2xl p-4 md:p-6 flex flex-col gap-4 md:gap-6 transition-all duration-200 hover:shadow-lg bg-white shadow-sm ${
                    isMilestoneProject 
                      ? 'border-purple-300 hover:border-purple-500' 
                      : 'border-gray-200 hover:border-purple-300'
                  }`}
                >
                  {/* Milestone Badge - Fixed positioning to avoid overlap */}
                  {isMilestoneProject && (
                    <div className="relative mb-2">
                      <span className="inline-block bg-gradient-to-r from-purple-500 to-blue-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                        📊 Milestone Project
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex-1">
                    <h3 className={`text-base md:text-[20px] font-semibold mb-1 md:mb-2 ${
                      isMilestoneProject ? 'pr-0' : 'pr-24 md:pr-14'
                    }`}>
                    ...
                    {/* Milestone Summary - Highlighted for milestone projects */}
                    {isMilestoneProject ? (
                      <div className="mt-3">
                        {renderMilestoneSummary(contract)}
                      </div>
                    ) : (
                      /* Single file project - show download buttons */
                      renderSingleFileButtons(contract)
                    )}
