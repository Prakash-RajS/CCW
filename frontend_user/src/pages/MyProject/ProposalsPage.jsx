...
<div className="flex-1 bg-white p-4 md:p-10">
  ...
  <div className="md:hidden sticky top-0 z-20 bg-white border-b border-gray-200 -mx-4 px-4 py-3 flex items-center gap-3 mb-4">
    <button
      onClick={() => window.history.back()}
      className="w-8 h-8 flex items-center justify-center rounded-full bg-[#6b4fa3] text-white hover:bg-[#5a3e8a] transition-colors flex-shrink-0"
    >
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <span className="font-semibold text-[#111] text-sm">Back</span>
    <div className="flex-1 text-right">
      <span className="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
        {activeTab === "proposals" ? `Proposals (${proposals.length})` : `Invitations (${invitations.length})`}
      </span>
    </div>
  </div>
  ...
  <div className="flex gap-2 mb-4 md:hidden">
    {["proposals","invitations"].map((tab) => (
      ...
      >
        {tab === "proposals" ? "Proposals" : "Invitations"}
      </button>
    ))}
  </div>
