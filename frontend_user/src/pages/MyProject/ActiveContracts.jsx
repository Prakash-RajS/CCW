// frontend_user/src/pages/MyProject/ActiveContracts.jsx
...
export default function ActiveContracts() {
  ...
  const fetchContracts = async () => {
    try {
      ...
      const response = await api.get("/contracts/by-status", {
        params: { status: "in_progress", user_id: userData.id },
      });
  ...
  // Render milestone progress for a contract
  const renderMilestoneProgress = (contract) => {
    if (!contract.milestones_data || contract.milestones_data.length === 0) {
      return null;
    }

    const milestones = contract.milestones_data;
    const totalPaid = milestones.filter(m => m.status === 'paid').reduce((sum, m) => sum + (m.amount || 0), 0);
    const currentMilestone = milestones.find(m => m.status === 'in_progress');

    return (
      <div className="mt-3 pt-3 border-t border-gray-200">
        <div className="flex justify-between items-center mb-2">
          <p className="text-xs font-semibold text-gray-600">Milestone Progress</p>
          <p className="text-xs text-gray-500">Paid: ₹{totalPaid} / ₹{contract.budget}</p>
        </div>
            ...
            <div key={idx} className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${
                  ...
                  milestone.status === 'revision_requested' ? 'bg-orange-500' :
                  'bg-gray-300'
                }`} />
                <span className="text-gray-700 truncate">{milestone.description}</span>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                <span className="text-gray-500 whitespace-nowrap">₹{milestone.amount}</span>
                {milestone.status === 'paid' && (
                  <span className="text-green-500 text-[10px] flex-shrink-0">✓</span>
                )}
                {milestone.status === 'in_progress' && idx === contract.current_milestone && (
                  <span className="text-blue-500 text-[10px] flex-shrink-0">Active</span>
                )}
              </div>
            </div>
          ))}
          {milestones.length > 3 && (
            <p className="text-xs text-gray-400">+{milestones.length - 3} more milestones</p>
          )}
        </div>
      </div>
    );
  ...
  <p className="text-[14px] md:text-[22px] mt-20 font-semibold hidden md:block">
    Total Budget: ₹{totalBudget} INR
          ...
          {/* TABS — no scroll, active tab highlighted */}
 ...
 <div className="flex justify-center text-[10px] sm:text-[11px] md:text-[13px] lg:text-[16px] xl:text-[17px] md:justify-center md:gap-1 lg:gap-2">
  {tabs.slice(0, 3).map((tab) => {
    ...
    return (
      <button
        ...
        className={`relative pb-2 pt-1 px-1.5 sm:px-2 md:px-2 lg:px-3 xl:px-4 whitespace-nowrap transition-colors duration-150 flex-1 md:flex-none ${isActive
 ...
 <div className="flex justify-center text-[10px] sm:text-[11px] md:text-[13px] lg:text-[16px] xl:text-[17px] md:justify-center border-t border-white/20 md:border-t-0 md:gap-1 lg:gap-2">
  {tabs.slice(3).map((tab) => {
    ...
    return (
      ...
      {/* CONTENT CARD - With improved spacing and dividers */}
      <div className="relative -mt-[30px] md:-mt-[90px] max-w-[1200px] mx-auto bg-white rounded-[18px] shadow-2xl p-4 md:p-8">
        {loading ? (
          ...
          <div className="text-center py-6 md:py-10">
            <p className="text-sm md:text-base text-gray-600">No active contracts found</p>
          </div>
        ) : (
          <>
            {latestJob && (
              <div className="mb-6 md:mb-8">
                <h3 className="text-base md:text-[20px] font-semibold text-[#1F1F1F] mb-2 md:mb-4">Your latest job</h3>
                  ...
                  <div className="relative pb-6 md:pb-0 h-full">
                    <div className="bg-white rounded-lg h-full flex flex-col">
                      ...
                      <div className="mt-auto">
                        {renderMilestoneProgress(contract)}
                      </div>

                      <div className="flex gap-3 md:gap-4 mt-4 md:mt-5">
                        <button
                          onClick={() => navigate("/editwork", {
                            state: { contractId: contract.id, fromTab: "activecontracts" },
                          })}
