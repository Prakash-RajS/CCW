...
const Header = ({ variant = "default" }) => {
  ...
  const getNotificationIcon = (notification) => {
    ...
    const getProfilePictureUrl = (user) => {
      ...
      } else if (user.profile_picture.startsWith('/media/')) {
        imageUrl = `${API_BASE_URL}${user.profile_picture}`;
      } else {
        imageUrl = `${API_BASE_URL}/media/${user.profile_picture}`;
      }
      return imageUrl;
    };
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (profilePicture && typeof profilePicture === 'string') {
        console.log("🔍 Raw profile_picture from backend:", profilePicture);

        try {
          profilePicture = decodeURIComponent(profilePicture);
          console.log("🔍 After decodeURIComponent:", profilePicture);
        } catch (e) {
          console.error("Failed to decode URL:", e);
        }

        if (profilePicture.includes(`${API_BASE_URL}/`)) {
          const parts = profilePicture.split(`${API_BASE_URL}/`);
          profilePicture = parts[parts.length - 1];
          console.log("🔍 After removing API base URL:", profilePicture);
        }

        if (profilePicture.includes('googleusercontent.com') &&
          !profilePicture.startsWith('http')) {
          profilePicture = `https://${profilePicture}`;
          console.log("🔍 Added https prefix:", profilePicture);
        }
      }
  ...
  const fetchUnreadMessageCount = async () => {
    try {
      ...
      if (response.data) {
        ...
        console.log(`💬 Setting unread message count to: ${count}`);
  ...
  const changeStatus = async (newStatus) => {
    ...
    try {
      await api.put(`/profile/update-status/${userData.id}/`, {
        status: mappedStatus
      });
      ...
      <nav className={`hidden md:flex w-auto lg:w-[609px] h-[52px] items-center justify-between rounded-[50px] px-2 lg:px-5 relative z-50 ${isLightVariant ? 'bg-white/90 backdrop-blur-[40px] border border-gray-200' : 'bg-[rgba(255,255,255,0.19)] backdrop-blur-[40px] border border-white'}`}>
        {navItems.map((item) => (
          ...
          >
            ...
            {/* My Project Dropdown - WHITE ON /finder-profile */}
  ...
  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs px-3 py-1.5 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap z-[100] pointer-events-none">
    {getFullName()}
      ...
      {/* PROFILE DROPDOWN (DESKTOP) - WHITE ON /finder-profile */}
    {/* COMBINED PROFILE & STATUS DROPDOWN (DESKTOP) - ENHANCED DESIGN */}
{isProfileMenuOpen && (
  <div
    className="
      absolute
  ...
  >
    ...
    <button 
      ...
      type="button"
      className={`
        w-full px-5 py-2.5 flex items-center gap-3 transition-all duration-200
        ${isFinderProfile || isLightVariant 
          ? 'text-gray-700 hover:bg-gray-200' 
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-white flex-shrink-0 flex-wrap md:flex-nowrap gap-2">
              <div className="flex items-center gap-1.5 flex-wrap sm:flex-nowrap">
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium rounded-full transition-all duration-200 whitespace-nowrap ${activeNotifyTab === "unread"
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium rounded-full transition-all duration-200 whitespace-nowrap ${activeNotifyTab === "read"
                    ? "bg-[#51218F] text-white shadow-md"
                    : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}
                >
              ...
              {loadingNotifications ? (
                ...
                filteredNotifications.map((notification, index) => (
                  <div
                    ...
                    className={`flex items-start gap-2.5 px-4 py-2.5 cursor-pointer transition-all duration-200 group ${!notification.is_read
