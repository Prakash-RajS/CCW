...
const CustomDropdown = ({
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {isOpen && !disabled && (
        <div
          className={`absolute z-50 w-full mt-1 rounded-lg shadow-lg overflow-hidden ${isDarkMode
        ...
        >
          {options.map((option) => (
            <button
              ...
              className={`w-full px-4 py-2 text-left text-[14px] transition-colors ${isDarkMode
...
const SubscriptionPage = () => {
  ...
  const triggerDashboardUpdate = (action = 'update', planName = '') => {
    dataUpdateEmitter.emit('subscriptionDataChanged', { action, source: 'SubscriptionPage', planName });
    dataUpdateEmitter.emit('dashboardDataUpdated', { source: 'SubscriptionPage', action, planName });
    localStorage.setItem('dashboardRefresh', Date.now().toString());
    localStorage.setItem('adminDataUpdated', Date.now().toString());
    console.log(`Dashboard update triggered: ${action} subscription plan - ${planName}`);
  };
...
const downloadInvoice = async (row) => {
  ...
  const openEditModal = (plan) => {
    ...
    setPlanForm({
      name: plan.name,
      price: plan.price_value || plan.original_price || plan.price_display.replace(/[^0-9.]/g, ""),
...
const { duplicateByName, duplicateByPrice } =
  checkDuplicatePlan(
    ...
    if (duplicateByPrice) {
      const roleText = planForm.role === "creator" ? "Creator" : "Collaborator";
      toast.error(`A plan with price ₹${planForm.price} for ${roleText} already exists! Please choose a different price.`);
      return;
    }
...
const { duplicateByName, duplicateByPrice } =
  checkDuplicatePlan(
    ...
    try {
      ...
      await api.put(`${PLANS_API_URL}/admin/edit-plan/${selectedPlan.id}`, planData);
  ...
  const confirmDeletePlan = async () => {
    ...
    try {
      await api.delete(`${PLANS_API_URL}/admin/delete-plan/${planToDelete.id}`);
  ...
  const getFilteredPlans = () => {
    ...
    return [...filtered].sort((a, b) => (a.price_value ?? 0) - (b.price_value ?? 0));
  };
  ...
  const totalHistoryPages = Math.ceil(sortedHistory.length / historyRowsPerPage);
  ...
  const endHistoryIndex = Math.min(historyCurrentPage * historyRowsPerPage, sortedHistory.length);
  const startPlansIndex = (plansCurrentPage - 1) * plansRowsPerPage + 1;
  const endPlansIndex = Math.min(plansCurrentPage * plansRowsPerPage, filteredPlans.length);
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
  ...
  <div
    ...
    style={isDarkMode ? {} : { background: "linear-gradient(90deg, #37264a 0%, #020202 100%)" }}
  >
    <div className="flex-shrink-0">
      <Users size={26} className={isDarkMode ? "text-purple-400" : "text-white"} />
    </div>
    <div className="flex flex-col items-start">
      <h2 className="text-3xl font-medium leading-none tracking-tight">
        {isLoading.stats ? <Loader className="animate-spin" size={24} /> : stats.total_subscribers}
      </h2>
      <p className="text-[13px] mt-1 capitalize opacity-90" style={{ fontFamily: "'Old Standard TT', serif" }}>
        Total Subscribers
      </p>
    </div>
  </div>
        ...
        {!isLoading.plans && pricingPlans.length > 0 && (
          <div className="flex gap-2 mb-6 flex-wrap">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition ${planRoleFilter === "all"
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition flex items-center gap-2 ${planRoleFilter === "creator"
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition flex items-center gap-2 ${planRoleFilter === "collaborator"
          ...
          {isLoading.plans ? (
            ...
            paginatedPlans.map((plan, idx) => (
              <div
                ...
                style={isDarkMode ? {} : { background: "linear-gradient(180deg, #7A5C97 0%, #6A4E87 100%)" }}
                onMouseEnter={() => setHoveredPlanId(plan.id)}
                onMouseLeave={() => setHoveredPlanId(null)}
              >
                <div className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold text-white ${getRoleBadgeColor(plan.role)}`}>
                ...
                {hoveredPlanId === plan.id && (
                  <div className="absolute top-4 right-4 flex gap-2">
                    ...
                    <button
                      onClick={() => deletePlan(plan.id, plan.name, plan.is_basic_or_free)}
    ...
    : "Monthly"}
                ...
                <div className="mb-4">
                  {plan.price_display !== "Free" ? (
                    <div className="text-2xl font-bold text-white">{plan.price_display}</div>
                  ) : (
                  ...
                  <div className="space-y-3 max-h-[250px] overflow-y-auto pr-2">
                    {plan.features && plan.features.map((feature, fIdx) => (
                      <div key={fIdx} className="flex items-start gap-2">
                        <div className={`w-4 h-4 min-w-[16px] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${isDarkMode ? "border-2 border-purple-400" : "border border-white"}`}>
                          <CheckCircle size={10} className={isDarkMode ? "text-purple-400" : "text-white"} />
                        </div>
                        <span className={`text-xs leading-relaxed break-words hyphens-auto ${isDarkMode ? "text-gray-300" : "text-white"}`}>
                ...
                <div className="mt-4 pt-3 border-t border-white/20">
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs opacity-70 text-white">Invites:</span>
                      <span className="text-sm font-bold text-white">{plan.max_invitations || "∞"}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <FileSignature size={12} className="opacity-70 text-white" />
                      <span className="text-xs opacity-70 text-white">Contracts:</span>
                      <span className="text-sm font-bold text-white">{plan.max_contracts || "∞"}</span>
                    </div>
                  </div>
                  <div className="flex items-baseline gap-2 mb-2">
                    <span className="text-2xl font-bold text-white">{plan.users || "0"}</span>
                    <span className="text-xs opacity-70 text-white">Active User</span>
                  </div>
                </div>
              </div>
            ))
            ...
            <div className="col-span-3 text-center py-12">
              <p className="text-gray-500 mb-4">
                {pricingPlans.length === 0 ? "No subscription plans available" : `No ${planRoleFilter} plans available`}
              </p>
              <button
                onClick={openCreateModal}
        ...
        {!isLoading.plans && filteredPlans.length > plansRowsPerPage && (
          <div className={`flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 px-2 py-3 text-sm font-medium ${isDarkMode ? "text-white" : "text-black"}`}>
            <div className={`flex items-center gap-2 order-2 sm:order-1 ${isDarkMode ? "text-white/80" : "text-black/80"}`}>
              ...
              <span>{filteredPlans.length > 0 ? `${startPlansIndex}-${endPlansIndex} of ${filteredPlans.length} plans` : "0 plans"}</span>
            </div>
            <div className="flex items-center gap-2 order-1 sm:order-2">
              ...
              {totalPlansPages > 0 && [...Array(Math.min(5, totalPlansPages))].map((_, i) => {
                let pageNum;
                ...
                return (
                  <button
                    ...
                    style={plansCurrentPage === pageNum ? { background: "linear-gradient(90deg, #51218F 0%, #020202 100%)" } : {}}
                  >
                    {pageNum}
                  </button>
                );
              ...
              <button
                onClick={() => handlePlansPageChange(plansCurrentPage + 1)}
                disabled={plansCurrentPage === totalPlansPages || totalPlansPages === 0}
                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed ${isDarkMode ? "border-white text-white hover:bg-gray-100/10" : "border-black text-black hover:bg-black/10"}`}
              >
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 backdrop-blur-[2px] p-4">
            <div className={`rounded-[20px] p-6 w-full max-w-2xl shadow-2xl relative ${isDarkMode ? "bg-gray-900" : "bg-gray-100"}`}>
              <div className="flex justify-between items-center mb-5 pb-3 border-b border-gray-200 dark:border-gray-700">
                <h3 className={`text-xl md:text-[22px] font-bold ${isDarkMode ? "text-white" : "text-[#1B2559]"}`}>
                  {modalMode === "create" ? "Create New Plan" : `Edit ${selectedPlan?.name || ""} Plan`}
                </h3>
                <button
              ...
              >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (touchedFields.price) {
                          setValidationErrors({ ...validationErrors, price: validatePrice(value) });
                        }
                      }}
                      onBlur={() => {
                        handleFieldBlur("price");
                        setValidationErrors({ ...validationErrors, price: validatePrice(planForm.price) });
                      }}
                      style={getInputStyle(isDarkMode)}
                      placeholder="0.00"
                    />
                    {touchedFields.price && validationErrors.price && (
                      <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><span>⚠️</span> {validationErrors.price}</p>
                    )}
                  </div>
                  <div>
                    <CustomDropdown
                      value={planForm.duration}
                      onChange={(value) => setPlanForm({ ...planForm, duration: value })}
                      options={[
                ...
                <div>
                  <CustomDropdown
                    value={planForm.role}
                    onChange={(value) => setPlanForm({ ...planForm, role: value })}
                    options={[
                ...
                <div className="border-t pt-4 border-gray-200 dark:border-gray-700">
                  <h3 className={`text-lg font-medium mb-4 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Plan Limits</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Max Users</label>
                      <input type="text" value={planForm.max_users} onChange={(e) => { const sanitized = sanitizeNumericInput(e.target.value); setPlanForm({ ...planForm, max_users: sanitized || "" }); }} style={getInputStyle(isDarkMode)} placeholder="Enter number" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Storage (GB)</label>
                      <input type="text" value={planForm.max_upload_storage_gb} onChange={(e) => { const sanitized = sanitizeNumericInput(e.target.value); setPlanForm({ ...planForm, max_upload_storage_gb: sanitized || "" }); }} style={getInputStyle(isDarkMode)} placeholder="Enter number" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Max Proposals</label>
                      <input type="text" value={planForm.max_proposals} onChange={(e) => { const sanitized = sanitizeNumericInput(e.target.value); setPlanForm({ ...planForm, max_proposals: sanitized || "" }); }} style={getInputStyle(isDarkMode)} placeholder="Enter number" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Max Job Posts</label>
                      <input type="text" value={planForm.max_job_posts} onChange={(e) => { const sanitized = sanitizeNumericInput(e.target.value); setPlanForm({ ...planForm, max_job_posts: sanitized || "" }); }} style={getInputStyle(isDarkMode)} placeholder="Enter number" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Max Invitations</label>
                      <input type="text" value={planForm.max_invitations} onChange={(e) => { const sanitized = sanitizeNumericInput(e.target.value); setPlanForm({ ...planForm, max_invitations: sanitized || "" }); }} style={getInputStyle(isDarkMode)} placeholder="Enter number" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>Max Contracts</label>
                      <input type="text" value={planForm.max_contracts} onChange={(e) => { const sanitized = sanitizeNumericInput(e.target.value); setPlanForm({ ...planForm, max_contracts: sanitized || "" }); }} style={getInputStyle(isDarkMode)} placeholder="Enter number" />
                    </div>
                  </div>
                </div>
    ...
    <p className="text-red-500 text-xs mt-1">
      {validationErrors.discount_percentage}
                ...
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="is_popular" checked={planForm.is_popular} onChange={(e) => setPlanForm({ ...planForm, is_popular: e.target.checked })} className="w-5 h-5 rounded border-2 border-gray-400" />
                  <label htmlFor="is_popular" className={`text-sm ${isDarkMode ? "text-white" : "text-gray-700"}`}>Mark as Popular Plan</label>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-6 mt-4 border-t border-gray-200 dark:border-gray-700">
                <button
                  ...
                  style={{ background: "linear-gradient(90deg, #51218F 0%, #020202 100%)" }}
                >
                  {isLoading.action ? <Loader className="animate-spin mx-auto" size={20} /> : (modalMode === "create" ? "Create Plan" : "Update Plan")}
                </button>
                <button
                  onClick={() => { setIsPlanModalOpen(false); resetPlanForm(); }}
        ...
        <div className="mt-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-wrap mb-4">
            <div className="flex gap-2 flex-wrap flex-1">
              ...
              >
                <Download size={14} /> Paid Plans ({subscriptionHistory.filter((i) => !i.is_basic).length})
              </button>
              <button
              ...
              >
                <User size={14} /> Basic/Free ({subscriptionHistory.filter((i) => i.is_basic).length})
              </button>
            </div>
            ...
            {isLoading.history ? (
              ...
              <>
                <div className="overflow-x-auto">
                  <table className="w-full text-left min-w-[1100px]" style={{ borderCollapse: "collapse" }}>
                    <thead>
                      <tr
                      ...
                      >
                        <th className="py-3 px-4 text-[13px] font-semibold">S.No</th>
                        ...
                        <th className="py-3 px-4 text-[13px] font-semibold">End Date</th>
                        <th className="py-3 px-4 text-[13px] font-semibold">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {paginatedHistory.map((user, idx) => {
                        const globalIndex = (historyCurrentPage - 1) * historyRowsPerPage + idx + 1;
                        const cycleValue = user.billing_cycle || user.duration || "monthly";
                        const isYearly =
                          cycleValue === "yearly" ||
                          cycleValue === "Yearly" ||
                          cycleValue === "annual" ||
                        ...
                        return (
                          <tr
                            ...
                            style={{
                              borderBottom: idx === paginatedHistory.length - 1
                                ? "none"
                                : isDarkMode
                                  ? "1px solid #374151"
                                  : "1px solid #9ca3af",
                          ...
                          >
                            <td className="py-3 px-4 font-medium text-[13px]">{globalIndex}</td>
                            <td className="py-3 px-4">
                              <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-full border overflow-hidden shrink-0 ${isDarkMode ? "border-white/50" : "border-gray-200"}`}>
                                  <Avatar user={user} />
                                </div>
                                <span className="font-semibold whitespace-nowrap text-[13px]">{user.full_name}</span>
                              </div>
                            </td>
                            ...
                            <td className="py-3 px-4 font-bold text-[13px]">
                              <div className="flex items-center gap-2">
                                {user.plan}
                                {user.is_basic && (
                                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${isDarkMode ? "bg-gray-700 text-gray-300" : "bg-gray-200 text-gray-600"}`}>
                                    Free
                                  </span>
                                )}
                              </div>
                            ...
                            <td className="py-3 px-4 text-[13px]">
                              <span className={`px-2 py-1 rounded-full text-[11px] font-medium ${isYearly ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}>
                                {cycleDisplay}
                              </span>
                            </td>
                            <td className="py-3 px-4 text-[13px]">{formatDate(user.start_date)}</td>
                            <td className="py-3 px-4 text-[13px]">{formatDate(user.end_date)}</td>
                            <td className="py-3 px-4">
                              {!user.is_basic ? (
                                <button
                                ...
                                >
                                  {isLoading.action && selectedRow?.id === user.id ? (
                                ...
                                <span className="text-gray-400 text-[11px] italic px-2">No invoice</span>
                              )}
                            </td>
                          </tr>
                        );
                ...
                <div className={`flex flex-col sm:flex-row justify-between items-center gap-4 px-4 py-4 text-sm font-medium ${isDarkMode ? "text-white" : "text-black"}`}>
                  <div className={`flex items-center gap-2 order-2 sm:order-1 ${isDarkMode ? "text-white/80" : "text-gray-600"}`}>
                    ...
                    <select
                      ...
                      onChange={(e) => handleHistoryRowsPerPageChange(e.target.value)}
                    ...
                    <span>{sortedHistory.length > 0 ? `${startHistoryIndex}-${endHistoryIndex} of ${sortedHistory.length} rows` : "0 rows"}</span>
                  </div>

                  <div className="flex items-center gap-2 order-1 sm:order-2">
                    <button
                      onClick={() => handleHistoryPageChange(historyCurrentPage - 1)}
                      disabled={historyCurrentPage === 1 || totalHistoryPages === 0}
                    ...
                    {totalHistoryPages > 0 && [...Array(Math.min(5, totalHistoryPages))].map((_, i) => {
                      let pageNum;
                      if (totalHistoryPages <= 5) pageNum = i + 1;
                      else if (historyCurrentPage <= 3) pageNum = i + 1;
                      else if (historyCurrentPage >= totalHistoryPages - 2) pageNum = totalHistoryPages - 4 + i;
                      ...
                      return (
                        <button
                          ...
                          style={historyCurrentPage === pageNum ? { background: "linear-gradient(90deg, #4C1D95 0%, #5B21B6 100%)" } : {}}
                        >
                          {pageNum}
                        </button>
                      );
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 flex items-center justify-center z-[999]">
            ...
            <div className={`relative w-full max-w-md rounded-[24px] shadow-[0_0_20px_rgba(0,0,0,0.3)] p-6 text-center ${isDarkMode ? "bg-gray-900 border border-gray-700" : "bg-gray-100/95 backdrop-blur-lg"}`}>
              <h3 className={`text-2xl font-semibold mb-3 ${isDarkMode ? "text-white" : "text-black"}`}>Delete Plan</h3>
